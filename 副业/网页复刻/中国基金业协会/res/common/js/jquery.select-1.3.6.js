...
(function($) {
    $.fn.extend({
        sSelect: function() {
            ...
            return this.each(function(i, obj) {
                ...
                if (obj.style.display != 'none' && $(this).parents()[0].id.indexOf('__jQSelect') < 0) {
                    ...
                    var titleWidth = $('#' + selectId).parents('.organ').find('.title').width() || 0; //
                    var optionWidth = $('#' + selectId).parents('td').width() - titleWidth - 40;
                    $('#' + selectId + ' h4').css({
                    ...
                    $.fn.setSelectValue(selectId);
                    var selectWidth = $('#' + selectId + ' select').width();
                    if ($.browser.safari && ((navigator.platform == "Win32") || (navigator.platform == "Windows"))) {
                        selectWidth = selectWidth + 25
                    }
                    $('#' + selectId + ' h4').css({
                        width: selectWidth
                    });
        ...
        reset: function() {
            return this.each(function(i, obj) {
                if ($(this).hasClass('mod-select')) {
                    var defaultOption = $(this).find('option:first');
                    if (defaultOption) {
                        var defaultVal = $(defaultOption).val();
                        $(this).val(defaultVal).parent().find('h4').html($(defaultOption).html());
                    } else {
                        $(this).val('').parent().find('h4').html('全部');
                    }
                    $(this).trigger('change');
                } else if ($(this).hasClass('textParam')) {
                    $(this).val($(this).attr('placeholderVal')).addClass("tipcolor");
                } else {
                    $(this).val('');
                }
            });
        },
        getFinalVal: function() {
            var originVal = $.trim($(this).val());
            if ($(this).hasClass('textParam') && originVal == $(this).attr('placeholderVal')) {
                originVal = null;
            }
            return originVal;
        }
